    ...
    def train(self, opt):
        ...
        with sv.managed_session(config=config) as sess:
            ...
            for step in range(1, opt.max_steps):
                ...
                ############ IMPORTANT ##############
                # training using NYU is based on the pretrained KITTI model
                # in the pretrained model has been run for 190532 global steps
                # need to offset gs to reflect the global steps which current model has run with NYU
                gs = gs - 190532
